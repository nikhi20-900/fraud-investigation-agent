...
The Agentic Fraud Investigation Agent is an enterprise-grade forensic intelligence system that combines graph relationship analytics, deterministic heuristic detection, autonomous agent reasoning, independent risk/uncertainty scoring, and auditable action prioritization into a single cohesive pipeline.
...
| Quadrant | Risk | Uncertainty | Recommended Operational Posture |
|:---|:---:|:---:|:---|
| **High Risk, Low Uncertainty** | High | Low | **Immediate Containment:** Freeze accounts, block IPs, submit SAR. |
| **High Risk, High Uncertainty** | High | High | **Urgent Investigation:** Priority escalation, request telemetry, verify identities. |
| **Low Risk, Low Uncertainty** | Low | Low | **Clean Baseline:** No intrusive action, normal automated monitoring. |
| **Low Risk, High Uncertainty** | Low | High | **Information Gathering:** Passive enrichment, monitor future activity. |

### 2.4 Why Recommendations are Deterministic
Operational actions (e.g., requesting biometric KYC, placing funds holds, freezing debit cards) have direct customer-impact and legal consequences. Generating recommendations via unstructured LLM prompts risks non-deterministic hallucinations, duplicate suggestions, and volatile priority inversions.
...
### 2.5 Why the Simulator Fallback Exists
...
The system includes a fully functional, in-memory **Python Graph Simulator** (`backend/app/services/graph_service.py`). The simulator mirrors TigerGraph's GSQL query semantics:
- Loads the identical 133-vertex, 147-edge synthetic schema.
- Executes identical 2-hop neighborhood expansion, co-location queries, and layering traces.
- Enables 100% offline local development, zero-dependency CI/CD execution, and reproducible evaluation tests.

### 2.6 Where TigerGraph Fits
In a live production environment, TigerGraph serves as the primary system of record for relationship intelligence:
- Stores the global financial entity graph (Customers, Accounts, Cards, Devices, IPs, Merchants, and connecting edges).
- GSQL queries (`graph/queries/`) execute distributed pattern detection at scale directly inside the database engine.
- The FastAPI backend communicates with TigerGraph via RESTPP endpoints using secure credential tokens.

### 2.7 Where LangGraph Fits
LangGraph provides the stateful multi-step agentic reasoning loop (`agent/agent.py`):
- Coordinates an autonomous investigative cycle: **Planner** $\to$ **Investigator** $\to$ **Evidence Analyzer** $\to$ **Report Generator**.
...
1. **Frontend Presentation Layer (`frontend/`)**:
   ...
   - Tailored Apple-inspired design system with dark mode glassmorphism, fluid typography, and lucide-react icons.
...
6. **Evaluation & Verification Harness (`tests/evaluation/`)**:
   - 91 automated tests validating correctness, determinism, grounding integrity, and performance baselines.
...
- **Test Suite:** **91/91 tests passing** (0 regressions).
